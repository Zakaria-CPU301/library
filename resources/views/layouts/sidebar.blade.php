<div class="px-3 p-5">
    ...
    <ul class="space-y-2 font-medium">
        ...
        @endif
        <li>
            <x-nav-link :href="route('tools.' . $currentUser->role)" :active="request()->routeIs('tools.*') && !request()->routeIs('tools.category')">
        ...
        @if ($currentUser->role === 'admin')
            <li>
                <x-nav-link :href="route('tools.category')" :active="request()->routeIs('tools.category')">
                    <i class="bi bi-tag text-3xl"></i>
                    <span x-show="open" class="flex-1 ms-3 whitespace-nowrap">Kelola Kategori</span>
                </x-nav-link>
            </li>
        @endif
